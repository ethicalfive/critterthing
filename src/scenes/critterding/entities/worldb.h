...
// 		void		initShared();
		...
		virtual btVector3 	findPositionNew();
...
// 		void			drawWithinCritterSight(unsigned int cid);
		...
		void			insertRandomFood(int amount, float energy, unsigned int totalFrames);

		virtual void		insertCritter();
		
		virtual void		insertCritterNew();	//ADDED for critterthing
		
		void			saveAllCritters();
		virtual void		loadAllCritters();
		virtual void		loadAllLastSavedCritters();
		virtual void 		loadLastProfile();
...
// 		btTransform m_follow_critter_transform;
	...
	protected:
		...
		boost::shared_ptr<ScClientMapResource>	m_arimaa_board;
...
// 		inline void		createDirs();
		...
		const int*	population_eliminate_portion_toggle_exchange_rand;
		const int*	population_rand_toggle;
		...
		const int* 	fitness_height;
		const int* 	fitness_distance;
		const int* 	fitness_jump;
