...
// 		retinasperrow = settings->getCVarPtr("retinasperrow");
		...
		//fitness
		fitness_height = settings->getCVarPtr("fitness_height");
		fitness_distance = settings->getCVarPtr("fitness_distance");
		fitness_jump = settings->getCVarPtr("fitness_jump");
...
// 		headless = settings->getCVarPtr("headless");
		...
		population_eliminate_portion_toggle_exchange_rand = settings->getCVarPtr("population_eliminate_portion_toggle_exchange_rand");
		population_rand_toggle = settings->getCVarPtr("population_rand_toggle");
		//save_and_close_sim_at_x_food = settings->getCVarPtr("save_and_close_sim_at_x_food");
	...
	critterSize 		= settings->getCVar("body_maxbodypartsize");
	insertHeight		= 6.0f * ( critterSize / 50 );
...
void WorldB::init()
{
	...
	//	std::cout << "autoloadlastsaved" << std::endl;
	}
	else
	{
		makeFloor();
...
void WorldB::calcFreeEnergy()
{
	...
	{
		m_energy_ratio = 0.01f * *population_limit_energy_percent;  //ethical retur here
...
// 			nbody.process();
		...
		//Map resized?
		//	if ( worldsizeX != settings->getCVarPtr("worldsizeX") || worldsizeY != settings->getCVarPtr("worldsizeY") ||worldsizeZ != settings->getCVarPtr("worldsizeZ") )
		//		makeFloor();
		...
		// Autoexchange Critters?
			if (*population_rand_toggle == 0 )
			{
				autoexchangeCritters(timer);
			}
			//else if (population_rand_toggle == 1 );
			//{
...
// 			np.setX(np.getX()+1.0f);
		...
		if ( randgen->Instance()->get(1,100) <= Settings::Instance()->getCVar("critter_mutate_carnivore") && Settings::Instance()->getCVar("critter_enablecarnivores") == 1 ) //ADDED for critterthing
		{
			if ( c->genotype->crittertype == 1 )
				nc->crittertype = 0;
			else nc->genotype->crittertype = 1;
		}	
		else
			nc->genotype->crittertype = c->genotype->crittertype;
...
// 				cout << buf.str()<< endl;
		...
		// rejuvenate
		//	c->totalFrames *= 0.5f;           //disabled by ethical suspected abuse
...
void WorldB::eat( CritterB* c )
{
	if ( c->eat )
	{
		if ( c->touchingFood && c->genotype->crittertype == 0)
		{
...
//ADDED carnivores for critterthing, throwback from carnivores from CD9.1
		
		else if ( c->touchingCritter && c->genotype->crittertype == 1 )  
		{
			CritterB* ct = static_cast<CritterB*>(c->touchedEntity);
			float eaten = 0.05f * *critter_maxenergy;
			if ( c->energyLevel + eaten > *critter_maxenergy )
		...
		if ( Settings::Instance()->getCVar("critter_enablepoison") == 1 )
		{
			if ( c->touchingFood && c->genotype->crittertype ==1 ) //poison
			{
				Food* f = static_cast<Food*>(c->touchedEntity);
				float eaten = 0.05f * *critter_maxenergy;
				if ( c->energyLevel + eaten > *critter_maxenergy )
			...
			else if ( c->touchingCritter && c->genotype->crittertype == 0 )  //poison
			{
				CritterB* ct = static_cast<CritterB*>(c->touchedEntity);
				float eaten = 0.05f * *critter_maxenergy;
				if ( c->energyLevel + eaten > *critter_maxenergy )
...
void WorldB::killHalforDouble()
{
	if ( *population_double != 0 && critters.size() <= (unsigned int)*population_double )
	{
		...
		settings->setCVar("energy", *energy + 10 );  //ethical - bump energy level up to prevent death degrade spiral
	}
	else if ( critters.size() >= (unsigned int)*population_eliminate_portion )
	{
		killHalfOfCritters();
		
		if ( settings->getCVar("save_and_close_sim_at_x_food") )
		{
			settings->setCVar("population_double", 20 );
			settings->setCVar("mincritters", 0 );
		}
...
// 		}
		...
		if ( *population_eliminate_portion_toggle_exchange_rand > 0 )
		{
			settings->setCVar("population_rand_toggle", randgen->Instance()->get(0,1) );
		}
		...
		if ( *population_eliminate_portion_decrmaxlifetimepct > 0 )
		{
			...
			settings->setCVar("critter_maxlifetime", *critter_maxlifetime+dec ); //edit by ethical to plus instead of minus
		}
	}
}
...
// 	200     @     32000  = 160			>   1/160
	...
	if ( *food_maxenergy > 0 && *food_maxlifetime > 0 )	
	{
		...
		if (statsBuffer->current.food == 0 && *energy > 0  )		//ADDED BY Ethical
		{
			for (int i = 0, j = *food_maxlifetime; i < *energy; ++i)
			{
				insertRandomFood( 1  , *food_maxenergy, (unsigned int)j);
				m_freeEnergy -= *food_maxenergy;
				j -= ( *food_maxlifetime / *energy );
		...
		// 			if ( ++foodIntervalCounter >= exp_interval_div || queue <= 2 )
					{
						...
						for ( unsigned int i(0); i < number_to_insert; ++i )
						{
							if ( m_freeEnergy >= *food_maxenergy )
							{
								insertRandomFood(1, *food_maxenergy, 0);
								m_freeEnergy -= *food_maxenergy;
...
// 				cout << buf.str()<< endl;
	...
	// 		else if ( c->totalFrames > (unsigned int)*critter_maxlifetime )
			{
				stringstream buf;
				buf << setw(4) << c->critterID << " died of old age"; //edit by Ethical
				m_panel_textverbosemessage->addDeath(buf);
	...
	// 				cout << buf.str()<< endl;
			...
			else
			{
				...
				{
					stringstream buf;
					buf << setw(4) << c->critterID << " fell into the pit";
					m_panel_textverbosemessage->addDeath(buf);
...
void WorldB::insertRandomFood(int amount, float energy, unsigned int totalFrames) // ", unsigned int totalFrames" ADDED BY Ethical 
{
	for ( int i=0; i < amount; ++i )
	{
		Food *f = new Food();
		f->energyLevel = energy;
		f->totalFrames = totalFrames;			//ADDED BY Ethical
		//f->resize();
...
void WorldB::insertCritterNew()			//ADDED for critterthing
{
	btTransform t;
	t.setIdentity();
	t.setOrigin(findPosition());
...
{
	btVector3 resizing_offset( m_initial_worldsizeX - *worldsizeX, insertHeight  , m_initial_worldsizeZ - *worldsizeZ ); //change this after racemode vVVVV
...
btVector3 WorldB::findPositionNew()  //ADDED for critterthing - TODO find nearest location to spawn critter ob ground instead of drop in from above
{
	btVector3 resizing_offset( m_initial_worldsizeX - *worldsizeX, insertHeight  , m_initial_worldsizeZ - *worldsizeZ ); //change this after racemode vVVVV
	return btVector3( (float)randgen->Instance()->get( 0, 100**worldsizeX ) / 100, insertHeight , (float)randgen->Instance()->get( 0, 100**worldsizeZ ) / 100 ) + ( resizing_offset * 0.5f );
}
...
void WorldB::killHalfOfCritters()					//TODO -- change this function to *randomly* eliminate the population
{	
									//		to avoid effecting the selection proceses
	int rand = 0;
	for ( unsigned int c = 0; c < critters.size(); c++ )		//how does this select only half the critters when it should cycle through
	{								//	every critter?
		if ( randgen->Instance()->get(0,100) <= *population_eliminate_portion_percent )              // Partial Hack I need to fix as it only partially worksas intended
			removeCritter(c);				//want to change the killhalf to kill 12%
	}
	
	
	
	
	/* 									edit by Ethical -- Disabled remove by age suspecting critter abuse
	// kill oldest
	if ( critters.size() > 0 )
	{
...
void WorldB::drawFood(const btVector3* cposi)
{
	...
	btVector3 FoodSize(0.01f* *food_size, 0.01f * *food_size, 0.01f * *food_size);  //ADDED by Ethical
	if ( e_size > 0 )
	{
...
// 							glColor4f( e->color.r(), e->color.g(), e->color.b(), e->color.a() );
						...
						m_model_food->draw( 0, tr, FoodSize ); //EDITED by Ethical to include foodsize transformation 
					}
...
void WorldB::loadLastProfile() // Load Profile -- ADDED for critterthing
{
	vector<string> files;
...
// 			std::string content;
	...
	// 		fileH.open( filen, lastsaveddir ); 
		...
		// 	lastsaveddir.append("/");
			...
			for ( unsigned int i = 0; i < files.size(); ++i )
			{
				if ( parseH.endMatches( ".pro", files[i] ) )
				{
					stringstream buf;
					buf << "loading profile " << files[i];
					m_logBuffer->add(buf.str());
					std::string profile;
					//profile.append(lastsaveddir);
					profile.append(files[i]);
					//char* profile_obj[];
					//profile_obj[0] = lastsaveddir + files[i];
					cerr << "loaded profile: " << profile  << endl;
					Settings::Instance()->loadProfileLastSaved( profile );  //<----- TODO make this work with original function
...
// 			std::string content;
	...
	// 		fileH.open( filen, lastsaveddir ); 
		...
		// 	lastsaveddir.append("/");
			...
			for ( unsigned int i = 0; i < files.size(); ++i )
			{
				/*  moved outside function so critters will load inside worldsize area
				if ( parseH.endMatches( ".pro", files[i] ) )
				{
					stringstream buf;
					buf << "loading profile " << files[i];
...
// 		fileH.save(filename.str(), critters[i]->genotype->saveGenotype());
	...
	Settings::Instance()->saveProfile(subsavedir ); // ADDED by Ethical -- Save profile into critter save directory
...
void WorldB::duplicateCritter(int cid, bool brainmutant, bool bodymutant)
{
	if ( cid > -1 )
	{
		...
		//duplicate crittertype
		nc->genotype->crittertype = critters[cid]->genotype->crittertype;
...
//		btTransform t;
 ...
 //		m_arimaa_board=m_modelSystem->load(mapName, m_graphicsSystem, m_modelSystem, btVector3(2.0f,20.0f,0.5f), t);
//	}
