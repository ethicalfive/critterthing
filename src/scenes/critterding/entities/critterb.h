...
//		CritterBNew(btDynamicsWorld* btWorld, long unsigned int id, const btTransform& startPos, unsigned char* retinap);
//		~CritterBNew();
...
// 		void			releaseFBO();
		...
		float			fitness_avg_height;
		float			fitness_distance;
		float			fitness_best_height;
		float 			X,Y,Z;
