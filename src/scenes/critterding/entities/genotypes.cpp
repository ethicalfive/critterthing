...
// 	cerr << "loading genotype start" << endl;
	...
	while ( !line.empty() )
	{
		...
		// adamdist=690;
			...
			if ( parseH.beginMatchesStrip( "crittertype=", line ) )	//ADDED for critterthing
			{
				std::string CT = parseH.returnUntillStrip( ';', line );
				//cerr << "AD: " << AD << endl;
				if(EOF == sscanf(CT.c_str(), "%d", &gt->crittertype)) cerr << "ERROR INSERTING CRITTER" << endl;
			}			
