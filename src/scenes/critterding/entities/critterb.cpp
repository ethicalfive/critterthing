...
// 	critter_raycastvision = settings->getCVarPtr("critter_raycastvision");
	...
	visionFactor = settings->getCVarPtr("visionFactor");
	...
	fitness_avg_height = 0;
	fitness_distance = 0;
	fitness_best_height = 0;
...
/*
CritterB::CritterBNew(btDynamicsWorld* btWorld, long unsigned int id, const btTransform& startPos, unsigned char* retinap)
 : retinasperrow(119)
 , m_zNear(0.1f)
 , m_fovy(SIMD_HALF_PI/2)
 , m_frustumHalf(tan( m_fovy * 180.0f / M_PI * M_PI / 360.0f ) * m_zNear)
...
void CritterB::procInputNeurons()
{
	...
	// energy neurons
	{
		brain.Inputs[overstep++].output = energyLevel / *critter_maxenergy;
	...
	// age neurons
	{
		brain.Inputs[overstep++].output = (float)totalFrames / genotype->bodyArch->m_maxlifetime ;
...
// 		else
		{
			...
			for ( unsigned int h(retinaRowStart); h < val3; h += retinaRowLength )
			{
				...
				{
					brain.Inputs[overstep++].output = (*visionFactor*0.00000001) * retina[w];  //0.00390625f
				}
			}
		}
...
// 		cerr << endl;
	...
	//random input ADDED by Ethical
		if ( randgen->Instance()->get(1,10) > 5 ) 	brain.Inputs[overstep++].output = 1;
		else 						brain.Inputs[overstep++].output = 0;
...
//location
void CritterB::getLocation()
{
	btDefaultMotionState* cmyMotionState = (btDefaultMotionState*)body.mouths[0]->body->getMotionState();
