...
SamsMode::SamsMode(  boost::shared_ptr<BeGraphicsSystem> system, BeFilesystem& fileSystem, boost::shared_ptr<Textverbosemessage> textverbosemessage )
...
void SamsMode::init()
{
	...
	{
		loadLastProfile(); //initialise the profile before the floor is created
	...
	if ( settings->getCVar("autoload") )
		loadAllCritters();
//	cerr << endl << "autoloadingcritters = " << settings->getCVar("autoloadlastsaved") << endl;
	if ( settings->getCVar("autoloadlastsaved") )
	{
...
void SamsMode::process(const BeTimer& timer)
{
	if ( !pause )
	{
		++framecounter;
...
// 				freeEnergyc += c->energyUsed;
		...
		for( i=0; i < lmax; ++i)
		{
			...
			//	const btVector3& cposi = cmyMotionState->m_graphicsWorldTrans.getOrigin();
				...
				//if ( cposi.getY() > c->fitness_index )
					c->fitness_avg_height += c->Y;
					if ( c->Y > c->fitness_best_height )
						c->fitness_best_height = c->Y;
		...
		if ( (haveWinner || framecounter >= (unsigned int)*critter_maxlifetime) )
		{
			...
			cerr << "Evaluating..." << endl;
				...
				// measure their distances from their respective food targets
					...
					{
					if( Settings::Instance()->getCVar("fitness_height") )
						critters[i]->fitness_index = critters[i]->fitness_avg_height / critters[i]->totalFrames;
					if( Settings::Instance()->getCVar("fitness_jump") )
						critters[i]->fitness_index += critters[i]->fitness_best_height;
					if( Settings::Instance()->getCVar("fitness_distance") )
					{
						critters[i]->fitness_index += abs(critters[i]->X / 20);
						critters[i]->fitness_index += abs(critters[i]->Z / 20);
					}
						
						
					
					//	critters[i]->fitness_height = critters[i]->fitness_index;
...
// 					//		critters[i]->fitness_index += ( (float)*food_maxenergy /(f->energyLevel + 0.0000001));
				...
				// calc number to keep
					...
					else
					{
						...
						cerr << "we will eliminate " << critters.size() - bestNum << " critters" << endl;
	...
	// Wall Constants
		...
		*/
		// Ground Floor
		{
			...
			Wall* w = new Wall( *worldsizeX*20, WallWidth, *worldsizeZ*20, t, m_dynamicsWorld );
			w->m_model = m_model_cube;
			wall_units.push_back(w);
		}
		...
		{
			btVector3 position = btVector3( *worldsizeX/2.0f, -WallHalfWidth, *worldsizeZ/2.0f );
...
void SamsMode::resetCamera()
{
	if ( &m_sceneNodeCamera == m_camera.getSceneNode() )
	{
		...
		m_sceneNodeCamera.setOrigin( btVector3( 0.5f * *worldsizeX, 2.0f * *worldsizeY, 0.5f * *worldsizeZ) );
