...

{
	...
	int console_width = 20 + ((v_diam+spacing) * settings->getCVar("critter_retinasize") * 4);
	if ( console_width < 600 )
		console_width = 600;

	int console_height = 20 + ((v_diam+spacing) * 40);
	if ( console_height < 550 )
		console_height = 550;

	m_dimensions.set(20 + console_width, 80 + console_height);

	int buttons_starty = getHeight() - console_height - 10;
	int console_starty = buttons_starty;
	...
	// view widgets
	console = addWidgetButton( "bv_bview", Vector2i(10, m_dimensions.m_y+console_starty), Vector2i(console_width, console_height), "", Vector2i(0, 0), BeCommand(""), EVENT_NOREPEAT, 0 );
...
// 	BeWidgetPtr w = addWidgetButton( "bv_close", Vector2i(m_dimensions.m_x-25, 10), Vector2i(15, 15), "x", BeCommand("gui_togglepanel", "console"), EVENT_NOREPEAT, 0 );
	BeWidgetPtr w = addWidgetButton( "console_close", Vector2i(2, 2), Vector2i(12, 12), "x", BeCommand("gui_togglepanel", "console"), EVENT_NOREPEAT, 0 );
...
void Console::draw()
{
	...
	if (m_active)
	{
// 		console->getLocalPosition().m_x = m_dimensions.m_y+console_starty;
// 		console->getLocalPosition().m_y = m_dimensions.m_y+getHeight() - console_height - 10;
// 		Vector2i(10, m_dimensions.m_y+console_starty), Vector2i(console_width, console_height)
		
// 		m_dimensions.set(20 + console_width, 80 + console_height);
		
		console->setDimensions(Vector2i(m_dimensions.m_x-20, m_dimensions.m_y - 80));
...
// 			for ( unsigned int i=0; i < currentCritter->brain.numberOfInputs; ++i )
			{
				if ( (*b_inputs_it).id == 10000 )
				{
					...
					s.m_position_x = console->getLocalPosition().m_x;
				...
				else if ( (*b_inputs_it).id >= 50000 && (*b_inputs_it).id < 60000 )
				{
					...
					sensor s; s.sPointer = &(*b_inputs_it);
// 					s.position = Vector2f(console->getLocalPosition().m_x+((spacing+v_diam)*col)+10, 10.0f+((spacing+v_diam)*row));
					s.m_position_x = console->getLocalPosition().m_x+(((spacing/4)+v_diam)*col)+10;
...
// 					const double t(oneoverdistancesquared * *attractor3 / 1000);
			...
			const double maxx = console->getWidth()-v_radius - t_spacing;
			const double miny = sensors[start_of_vision_inputs].m_position_y + t_spacing;
			const double maxy = console->getHeight()-v_radius - t_spacing;

			m_motor_spacing = (float)console->getWidth( ) / m_num_motors;
...
// 				auto& n(neurons[i]);
				...
				else
				{
					...
					n.m_position_y = console->getHeight()-15;
// 					n.m_position_y = console->getHeight() / 2;
				}
...
// 				glColor4f(dimmed/2, 0.0f, dimmed/2, 1.0f);
				for ( unsigned int j=0; j < syn.size(); ++j )
				{
					...
					if ( as.isSensorNeuron && braininput.output )
					{
						...
						glVertex2f(n.m_position_x+console->getPosition().m_x,         n.m_position_y+console->getPosition().m_y);
						glVertex2f(console->getPosition().m_x+s_as.m_position_x,         console->getPosition().m_y+s_as.m_position_y);
					}
				}
				...
				for ( unsigned int j=0; j < syn.size(); ++j )
				{
					...
					if ( !as.isSensorNeuron && currentCritter->brain.Neurons[as.neuronID]->fired() )
					{
						...
						{
							glVertex2f(n.m_position_x+console->getPosition().m_x,         n.m_position_y+console->getPosition().m_y);
							glVertex2f(console->getPosition().m_x+n_as.m_position_x,         console->getPosition().m_y+n_as.m_position_y);
						}
					}
				}
...
// 						if ( as.isSensorNeuron )
// 						{
// 							glVertex2f(console->getPosition().m_x+sensors[as.realneuronID].m_position_x,         console->getPosition().m_y+sensors[as.realneuronID].m_position_y);
// 						}
// 						else
// 						{
// 							glVertex2f(console->getPosition().m_x+neurons[as.neuronID].m_position_x,         console->getPosition().m_y+neurons[as.neuronID].m_position_y);
// 						}
// 					}
// 				}
			}
...
// 				float nv_radius_width = 0.25f * v_radius;
				...
				glVertex2f(s.m_position_x+console->getPosition().m_x-nv_radius, s.m_position_y+console->getPosition().m_y+nv_radius);
				glVertex2f(s.m_position_x+console->getPosition().m_x-nv_radius, s.m_position_y+console->getPosition().m_y-nv_radius);
				glVertex2f(s.m_position_x+console->getPosition().m_x+nv_radius, s.m_position_y+console->getPosition().m_y-nv_radius);
				glVertex2f(s.m_position_x+console->getPosition().m_x+nv_radius, s.m_position_y+console->getPosition().m_y+nv_radius);
			}
			...
			for ( unsigned int i=0; i < n_size; ++i )
			{
				...
				if ( n.nPointer->motor() )
				{
					...
					if ( nv_radius < 0.5f )
						nv_radius = 0.5f;
					glVertex2f(n.m_position_x+console->getPosition().m_x-nv_radius, n.m_position_y+console->getPosition().m_y+nv_radius);
					glVertex2f(n.m_position_x+console->getPosition().m_x-nv_radius, n.m_position_y+console->getPosition().m_y-nv_radius);
					glVertex2f(n.m_position_x+console->getPosition().m_x+nv_radius, n.m_position_y+console->getPosition().m_y-nv_radius);
					glVertex2f(n.m_position_x+console->getPosition().m_x+nv_radius, n.m_position_y+console->getPosition().m_y+nv_radius);
				}
