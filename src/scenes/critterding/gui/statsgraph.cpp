...
void Statsgraph::draw()
{
	if (m_active)
	{
		...
		if ( settings->getCVar("sams_mode") == 1 )
		{
			if ( number > 0 )
			{
				...
				if ( statsBuffer->m_graph_highest > 0 )
				{
					...
					glBegin(GL_LINES);
						...
						{
							glVertex2f( position_offset_x + i, position_offset_y - (heightratio*statsBuffer->snapshots[i].fitnessindex) );
							glVertex2f( position_offset_x_plus + i, position_offset_y - (heightratio*statsBuffer->snapshots[i+1].fitnessindex) );
						}

					glColor3f(1.0f, 0.0f, 0.0f);
						for ( unsigned int i=start; i < number_minus_one; ++i )
						{
							glVertex2f( position_offset_x + i, position_offset_y - (heightratio*statsBuffer->snapshots[i].fitnessindextotal) );
							glVertex2f( position_offset_x_plus + i, position_offset_y - (heightratio*statsBuffer->snapshots[i+1].fitnessindextotal) );
						}
					glEnd();
				}
			}
