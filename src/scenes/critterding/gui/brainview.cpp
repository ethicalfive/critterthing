...
// 			for ( unsigned int i=0; i < currentCritter->brain.numberOfInputs; ++i )
			{
				...
				//PULSE NEURON TIMERS
				else if ( (*b_inputs_it).id == 10010 )
				{
					sverticalpos = -1;
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x+spacing;
					s.m_position_y = 10.0f+(spacing+v_diam)*sverticalpos;
					sensors.push_back(s);
				}
				else if ( (*b_inputs_it).id == 10011 )
				{
					sverticalpos = -1;
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x+spacing+spacing;
					s.m_position_y = 10.0f+(spacing+v_diam)*sverticalpos;
					sensors.push_back(s);
				}
				else if ( (*b_inputs_it).id == 10012 )
				{
					sverticalpos = -1;
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x+spacing+spacing+spacing;
					s.m_position_y = 10.0f+(spacing+v_diam)*sverticalpos;
					sensors.push_back(s);
				}
				else if ( (*b_inputs_it).id == 10013 )
				{
					sverticalpos = -1;
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x+spacing+spacing+spacing+spacing;
					s.m_position_y = 10.0f+(spacing+v_diam)*sverticalpos;
					sensors.push_back(s);
				}
				//!PULSE NEURON TIMERS
...
// 					n.m_links_to_neurons_sums.clear();
					...
					for ( unsigned int j=i+1; j < n_size; ++j )
					{
						...
						if (currentCritter->genotype->brainzArch->ArchNeurons[j].ArchSynapses.size() ) //ADDED by Ethical - prevents segfault for neurons with no synapses
						{
							...
							for ( unsigned int k=0; k < syn.size(); ++k )
							{
								...
								//std::cerr << " " << "as2.neuronID" << " j: " << j << " neuron number: " <<i << "k: " << k << std::endl;

								if ( !as2.isSensorNeuron && as2.neuronID == i )  //here
									nrlinks++;
...
// 					const double t(oneoverdistancesquared * *attractor3 / 1000);
					...
					#pragma omp critical
#endif
					if ( !n.nPointer->motor() )  //TODO motor neurons still exerting a pull on neuron cluster
					{
