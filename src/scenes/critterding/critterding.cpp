...
// 	1112992850
	...
	settings->registerCVar("sams_mode",							new CVar(0, 0, 1, false, "enable Sams Mode"));
...
// 	settings->registerCVar("testworld",							new CVar(0, 0, 1, false, "a world for test purposes"));
	settings->registerCVar("fitness_height",							new CVar(1, 0, 1, true, "select for height in Sams Mode"));
	settings->registerCVar("fitness_distance",							new CVar(0, 0, 1, true, "select for distance in Sams Mode"));
	settings->registerCVar("fitness_jump",							new CVar(0, 0, 1, true, "select for jumping height in Sams Mode"));
	...
	settings->registerCVar("skybox_rotationX",						new CVar(300, 0, 2147483647, false, "rotational speed of skybox along the X axis"));
...
// 	settings->registerCVar("worldwalls",							new CVar(1, 0, 1, false, "enable walls around the world"));
	...
	settings->registerCVar("mincritters",							new CVar(40, 0, 1000, false, "minimum number of critters"));

	settings->registerCVar("visionFactor",							new CVar(390625, 1, 2147483647, false, "multiplies retina input values by this value, (divided by 100,000,000)"));
...
// 	settings->registerCVar("retinasperrow",							new CVar(119, 1, 1000, false, "number of vision retinas to stack per row onscreen"));
	...
	settings->registerCVar("critter_sightrange",						new CVar(1000, 1, 2147483647, false, "distance a critter can see (10 = 1 worldsize)"));

	settings->registerCVar("critter_retinasize",						new CVar(20, 1, 50, false, "size of a critters eye retina"));
	...
	settings->registerCVar("critter_enablecarnivores",					new CVar(0, 0, 1, true, "enables critters a chance to exclusively eat each other"));
	settings->registerCVar("critter_mutate_carnivore",					new CVar(20, 0, 1000, true, "Chance of carnivore (permille)"));
	settings->registerCVar("critter_enablepoison",						new CVar(0, 0, 1, true, "makes critters poisonous to each other (food for carnivores)"));


	settings->registerCVar("save_and_close_sim_at_x_food",					new CVar(0, 0, 2147483647, false, "saves and closes simulation when simulation reaches some food value"));
...
// 	settings->registerCVar("critter_mutate_sightrange",					new CVar(0, 0, 2147483647, false, "chance of mutating the distance a critter can see"));
	...
	settings->registerCVar("body_minbodyparts",						new CVar(4, 0, 2147483647, false, "minimum body parts per critter"));
	settings->registerCVar("body_maxbodyparts",						new CVar(200, 0, 2147483647, false, "maximum body parts per critter"));
	settings->registerCVar("body_minbodypartsatbuildtime",					new CVar(4, 1, 2147483647, false, "minimum body parts for a new critter"));
	settings->registerCVar("body_maxbodypartsatbuildtime",					new CVar(12, 1, 2147483647, false, "maximum body parts for a new critter"));
	
	settings->registerCVar("body_maxconstraintangle",					new CVar(61415, 0, 2147483647, false, "maximum amount of rotation on a contraint"));
...
// 	settings->registerCVar("body_maxconstraintlimit",					new CVar(15706, 0, 2147483647, false, "maximum limit of a contraint"));
	settings->registerCVar("body_constraintpositionrange",			new CVar(100, 0, 100, false, "amount muscles can deviate from the center of a plane (%)"));
	...
	settings->registerCVar("body_percentmutateeffectchangecolor_slightly",				new CVar(100, 0, 100, false, "chance of slightly changing body color"));
...
// 	settings->registerCVar("body_percentmutateeffectchangemaxmutations",				new CVar(0, 0, 100, false, "chance of changing max mutations on body"));
	...
	settings->registerCVar("body_costhavingbodypart",					new CVar(0, 0, 2147483647, false, "cost of having a bodypart (1/100000 energy)"));
	settings->registerCVar("body_costtotalweight",						new CVar(0, 0, 2147483647, false, "cost multiplier of total body weight (1/100000 energy)"));
	...
	settings->registerCVar("brain_percentmutateeffectaddneuron",				new CVar(5, 0, 100, false, "chance of adding a neuron"));
	settings->registerCVar("brain_percentmutateeffectremoveneuron",				new CVar(5, 0, 100, false, "chance of removing a neuron"));
	settings->registerCVar("brain_percentmutateeffectalterneuron",				new CVar(5, 0, 100, false, "chance of altering a neuron"));
	settings->registerCVar("brain_percentmutateeffectaddsynapse",				new CVar(100, 0, 100, false, "chance of adding a synapse"));
	settings->registerCVar("brain_percentmutateeffectremovesynapse",			new CVar(100, 0, 100, false, "chance of removing a synapse"));
...
// 	settings->registerCVar("brain_percentmutateeffectchangemaxmutations",		new CVar(0, 0, 100, false, "chance of changing max mutations on brain"));
	...
	settings->registerCVar("brain_costhavingneuron",					new CVar(10, 0, 2147483647, false, "cost of having a neuron (1/100000 energy)"));
	...
	settings->registerCVar("population_eliminate_portion",						new CVar(100, 2, 2147483647, false, "eliminate a percentage of critters if population reaches n"));
	settings->registerCVar("population_eliminate_portion_percent",					new CVar(12, 1, 100, false, "eliminate n% of critters if population reaches population_eliminate_portion"));
	...
	settings->registerCVar("population_eliminate_portion_toggle_exchange_rand",					new CVar(0, 0, 1, false, "turn on/off auto_exchange randomly"));
	settings->registerCVar("population_rand_toggle",					new CVar(0, 0, 1, false, "exchange toggle"));
	
	
	settings->registerCVar("population_limit_energy",							new CVar(100, 0, 2147483647, false, "population limit"));
