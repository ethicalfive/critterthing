...
// 		std::cerr << "done removing obsolete brainstuff" << std::endl << std::endl;
	...
	void BrainzArch::addRandomArchNeuron()
	{
		...
		// determine firing threshold
			...
			unsigned int TempFiringThreshold = randgen->Instance()->get( minFiringThreshold, maxFiringThreshold*1000 );
							an.firingThreshold = TempFiringThreshold*0.001;
...
// 			else an.dendridicBranches = randgen->Instance()->get( 1, maxDendridicBranches );
			...
			//EXP CODE HERE
			int randselect = randgen->Instance()->get( 1, 3);
			if ( randselect == 1 )
				an.potentialDecay = 500;
			else if ( randselect == 2 )
				an.potentialDecay = 950;
			else
				an.potentialDecay = 1500;
	...
	unsigned int BrainzArch::addRandomArchSynapse(unsigned int parentneuron)
	{
		...
		// is it connected to a sensor neuron ?
			...
			//if ( randgen->Instance()->get( 1, 100 ) <= percentChanceSensorySynapse || archlayers[targetlayer].size() < 2 )
			if (  !ArchNeurons[parentneuron].isMotor && (randgen->Instance()->get( 1, 100 ) <= percentChanceSensorySynapse || ArchNeurons.size() < 2) )
			{
			...
			if ( mode <= modesum )
			{
				if ( ArchNeurons.size() > 0 )
				{
					...
					/* commented out by ethical
						...
						*/
						else if ( jmode == 2 )  //added by ethical
							runs++;
					...
					// firing threshold
						else if ( jmode == 5 )
						{
							...
							unsigned int TempFiringThreshold = randgen->Instance()->get( minFiringThreshold, maxFiringThreshold*1000 );
							ArchNeurons[nid].firingThreshold = TempFiringThreshold*0.001;
...
// 			}
		...
		// change a mutatable
			...
			if ( mode <= modesum )
			{
				...
				{
					unsigned int jmode = randgen->Instance()->get(1,2);
					unsigned int factor = randgen->Instance()->get(1,1000);

					if ( jmode == 1 && minFiringThreshold < maxFiringThreshold-factor )	minFiringThreshold+=factor*0.001; // watch out idd
					else if ( minFiringThreshold > 1+factor )				minFiringThreshold-=factor*0.001; // !!! > 1
				}
				...
				else if ( imode == 8 && mutate_maxFiringThreshold )
				{
					...
					if ( jmode == 1 && maxFiringThreshold < 1000-factor )		maxFiringThreshold+=factor*0.001;
					else if ( maxFiringThreshold > minFiringThreshold+factor )	maxFiringThreshold-=factor*0.001; // !!! > minFiringThreshold
				}
...
//				unsigned int nid = (ArchNeurons.size()-1);
				...
				if ( parseH.beginMatchesStrip( "f=", line ) )
				{
					...
					//cerr << "FT: " << FT  << std::endl;
					if(EOF == sscanf(FT.c_str(), "%f", &an.firingThreshold))		std::cerr << "ERROR INSERTING CRITTER" << std::endl; //ethical - changed to float
				}
