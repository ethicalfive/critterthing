...
//TEXT ENTRY
	...
	BeWidgetPtr BeWidgetContainer::addWidgetTextInput( const std::string& name, const Vector2i& pos, const Vector2i& dimensions, const std::string& textstring, const Vector2i& textpos, const BeCommand& cmds, EventProperty_Repeat repeat, unsigned int delay, const std::string& cvar  )
	{
		BeWidgetTextInputPtr t = BeWidgetTextInputPtr(new BeWidgetTextInput());
		t->setDimensions(dimensions);
		t->addWidget( "buttontext", textpos, BeWidgetTextPtr(new BeWidgetText(textstring))  );
		t->genSubmitEvent( textstring ,  cmds );
                t->setCVar(cvar);
