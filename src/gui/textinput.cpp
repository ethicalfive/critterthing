...
void BeWidgetTextInput::genSubmitEvent( const std::string& textstring, const BeCommand& cmds )
{
...
// 			submitEvent->getCommand()->m_argInt = m_text->getInt();
//                         submitEvent->getCommand()->m_argtype = BeCommand::A_INT;
			submitEvent->activate();
			submitEvent->process( BeEventSystem::Instance()->getCommandSystem(), 0 );
			

                        
                        if ( !m_cvar.empty() && m_text->getString() != "" )
			{
                            Settings::Instance()->setCVar(m_cvar, atoi(m_text->getString().c_str()));
			}

			int store = atoi(m_text->getString().c_str());
			if ( m_text->getString() != "" )
			{
				m_historyStrings.push_back(m_text->getString());
				m_historyIndex=m_historyStrings.size();
